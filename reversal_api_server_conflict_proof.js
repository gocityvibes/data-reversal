...
const pg = require('pg');              // <-- single import (no { Pool })
...
/* ===== DB PATCH (Neon/Postgres) — SINGLE COPY ONLY =====
   - Connects to Neon using DATABASE_URL (SSL on)
   - Adds /db/health and /db/debug
   - Avoids "already declared" issues by not declaring `Pool` at all
*/
if (!global.__dbInit) {
  ...
  // expose the pool so the rest of the file can use it
  global.__db = { pool };
  global.__dbInit = true;
}
/* ===== END DB PATCH ===== */
...
const config = {
  symbols: (process.env.SYMBOLS || 'ES=F').split(',').map(s => s.trim()),
  timeframes: (process.env.TIMEFRAMES || '1m,5m').split(',').map(t => t.trim()),
...
// Use the single pool from the DB patch
const pool = global.__db.pool;
...
class TechnicalIndicators {
  ...
  static atr(high, low, close, period = 14) {
    ...
    const avg = this.sma(trs.slice(1), period);
    for (let i = 0; i < avg.length; i++) out[i + period] = avg[i];
    return out;
  }
  ...
  static vwap(high, low, close, volume) {
    ...
    for (let i = 0; i < close.length; i++) {
      ...
      cumPV += tp * volume[i];
      cumV += volume[i];
      out.push(cumPV / Math.max(1, cumV));
    }
    return out;
  }
}
